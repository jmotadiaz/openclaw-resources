...
export class KayakTrainStrategy implements TrainScraperStrategy {
  ...
  async scrapeTrains(params: TrainSearchParams): Promise<TrainScraperResult> {
    ...
    try {
      ...
      for (let i = 0; i < 45; i++) {
        ...
        logger.info(`[KayakTrain] Progress poll ${i + 1}: done=${data.done} barFound=${data.found}`);
        if (data.done) { progressDone = true; break; }
        await sleep(2_000);
      }

      if (!progressDone) {
        throw new Error(`Search progress bar never completed after 90s — possible CAPTCHA or network issue`);
      }
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const data = this.parseResult(check);
        logger.info(`[KayakTrain] Cards poll ${i + 1}: cardsReady=${data.cardsReady} count=${data.realCount}`);
        if (data.cardsReady) { success = true; break; }
        await sleep(2_000);
      }
      ...
      if (!success) {
        ...
        logger.warn(`[KayakTrain] 0 results after progress done. Title: "${dumpData.title}". URL: ${url}`);
        return {
          status: 'error',
      ...
      if (rawResults.length === 0) {
        ...
        const dumpData = this.parseResult(dump);
        logger.warn(`[KayakTrain] 0 results. Title: "${dumpData.title}". URL: ${url}`);
        return {
          status: 'error',
      ...
      for (const r of rawResults) {
        ...
        if (typeof r.totalPrice === 'number' && r.totalPrice > 0) {
          ...
          const perPerson = parseFloat((r.pricePerPax ?? '').replace(/[^\d.,]/g, '').replace(',', '.'));
  ...
  private parseResult(raw: any): any {
    if (typeof raw?.result === 'string') {
      try { return JSON.parse(raw.result); } catch { return raw.result ?? {}; }
    }
